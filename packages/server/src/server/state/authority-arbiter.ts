 ...
 * Phase 4.4.3: Production atomic acceptance using FeltDB 0.5.1+ updateIfVersion()
 */
export class AuthorityArbiter {
   ...
   *
   * Phase 4.4.3 Deliverable: Atomic acceptance using durable CAS
   *
   * Flow:
   ...
   * 6. If accepted: update handoff status to "accepted" atomically via updateIfVersion()
  ...
  async atomicAccept(handoffId: string): Promise<AcceptanceResult> {
    ...
    if (existingDecision && existingDecision.winnerId) {
      ...
      if (isExistingActive) {
        ...
        if (handoff.metadata?.supersedes === existingDecision.winnerId) {
          ...
          // If it fails (conditional update fails due to version mismatch), another concurrent request beat us
          try {
            ...
            // Conditional update failed - another handoff already won or status changed
    ...
    // Try to atomically accept via conditional update using FeltDB 0.5.1+ updateIfVersion()
    // If it fails (version conflict), another concurrent request beat us - record rejection
    try {
      await this.repos.handoffs.accept(handoffId);
